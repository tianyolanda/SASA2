...
template <unsigned int block_size>
__global__ void furthest_point_sampling_weights_density_kernel(int b, int n, int m,
    const float *__restrict__ xyz, const float *__restrict__ weights, const float *__restrict__ density, float *__restrict__ temp, int *__restrict__ idxs) {
    // xyz: (B, N, 3)
    // weights: (B, N)
    // density: (B, N)
    // tmp: (B, N)
    ...
    xyz += batch_index * n * 3;
    weights += batch_index * n;
    density += batch_index * n;
    temp += batch_index * n;
    idxs += batch_index * m;

    //calculate weights multiply density as new weight
    float weights_with_density = 1;
    ...
    for (int k = tid; k < n; k += stride) {
        if (j == 0) {  // select the point with the largest weight in the first round
            ...
            d = min(d, temp[k]);
            temp[k] = d;
            weights_with_density = weights[k] * density[k];
            float d2 = d * max(weights_with_density, 1e-12);  // dist[old][k] * weights[k]
            besti = d2 > best ? k : besti;
            best = d2 > best ? d2 : best;
        }
    }
...
void furthest_point_sampling_weights_density_kernel_launcher(int b, int n, int m,
    ...
    switch (n_threads) {
        case 1024:
        furthest_point_sampling_weights_density_kernel<1024><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 512:
        furthest_point_sampling_weights_density_kernel<512><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 256:
        furthest_point_sampling_weights_density_kernel<256><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 128:
        furthest_point_sampling_weights_density_kernel<128><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 64:
        furthest_point_sampling_weights_density_kernel<64><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 32:
        furthest_point_sampling_weights_density_kernel<32><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 16:
        furthest_point_sampling_weights_density_kernel<16><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 8:
        furthest_point_sampling_weights_density_kernel<8><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 4:
        furthest_point_sampling_weights_density_kernel<4><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 2:
        furthest_point_sampling_weights_density_kernel<2><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        case 1:
        furthest_point_sampling_weights_density_kernel<1><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs); break;
        default:
        furthest_point_sampling_weights_density_kernel<512><<<b, n_threads>>>(b, n, m, xyz, weights, density, temp, idxs);
    }
