...
class DenseDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.sensor_type = self.dataset_cfg.get('SENSOR_TYPE', 'hdl64')
        self.signal_type = self.dataset_cfg.get('SIGNAL_TYPE', 'strongest')
        self.lidar_folder = self.dataset_cfg.get('LIDAR_FOLDER', f'lidar_{self.sensor_type}_{self.signal_type}')
        self.dense_infos = []

        suffix = '_vlp32' if self.sensor_type == 'vlp32' else ''
        split_dir = self.root_path / 'ImageSets' / f'{self.split}{suffix}.txt'
    ...
    def include_dense_data(self, mode):
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            if not info_path.exists():
                if self.logger is not None:
                    self.logger.warning('DENSE info file does not exist: %s' % info_path)
                continue
            with open(info_path, 'rb') as f:
                dense_infos.extend(pickle.load(f))
    ...
    def get_lidar(self, idx):
        ...
        return np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, self.dataset_cfg.NUM_POINT_FEATURES)
    ...
    def get_calib(self):
        calib_file = self.root_path / f'calib_{self.sensor_type}.txt'
        assert calib_file.exists(), f'{calib_file} not found'
        return calibration_kitti.Calibration(calib_file)

    @staticmethod
    def get_calib_from_info(info):
        calib = info['calib']
        return calibration_kitti.Calibration({
            'P2': calib['P2'][:3, :4],
            'R0': calib['R0_rect'][:3, :3],
            'Tr_velo2cam': calib['Tr_velo_to_cam'][:3, :4],
        })
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            info = {}
            pc_info = {'num_features': self.dataset_cfg.NUM_POINT_FEATURES, 'lidar_idx': sample_idx}
            ...
            info['calib'] = {'P2': p2, 'R0_rect': r0_4x4, 'Tr_velo_to_cam': v2c_4x4}
            ...
            if has_label:
                ...
                annotations['index'] = np.array(list(range(num_objects)) + [-1] * (num_gt - num_objects),
                                                dtype=np.int32)
                ...
                if count_inside_pts:
                    ...
                    pts_fov = points[get_fov_flag(pts_rect, info['image']['image_shape'], calib)]
                    corners_lidar = box_utils.boxes_to_corners_3d(gt_boxes_lidar)
                    num_points_in_gt = -np.ones(num_gt, dtype=np.int32)
                    for k in range(num_objects):
                        num_points_in_gt[k] = box_utils.in_hull(pts_fov[:, 0:3], corners_lidar[k]).sum()
    ...
    def __getitem__(self, index):
        ...
        points = self.get_lidar(sample_idx)
        calib = self.get_calib_from_info(info) if 'calib' in info else self.get_calib()
        img_shape = info.get('image', {}).get('image_shape', np.array([1024, 1920], dtype=np.int32))
